...
def measure_frame(path: Path) -> list[int]:
    ...
    for profile in profiles:
        ...
        if depth is not None and 2 <= depth < scan_depth:
            depths.append(depth)
    return depths
...
def detect_shared_frame(paths: list[Path]) -> int:
    ...
    measurements = [depth for index in sample_indices for depth in measure_frame(paths[index])]
...
def main() -> None:
    ...
    for source_path in selected:
        ...
        if output_path.exists() and not args.overwrite:
            skipped += 1
            continue
        prepare_photo(source_path, output_path, border)
        written += 1
        if written % 25 == 0:
